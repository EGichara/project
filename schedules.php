...
<div class="content py-5" >
    <div class="row justify-content-center">
        <div class="col-md-12">
            <div class="card card-outline card-primary rounded-0 shadow "style="border-color: green;" >
                <div class="card-body" >
                    
                    <div class="callout border-primary rounded-0 shadow"style="border:none;" >
                        <fieldset >
                            ...
                            <form action="" id="filter-schedule">
                                <div class="row align-items-end">
                                    ...
                                    <div class="col-md-3 col-sm-4" >
                                        <button class="btn btn-flat btn-primary" style="background-color: green;"><i class="fa fa-search"></i> Search</button>
                                    </div>
                                </div>
                            </form>
                        </fieldset>
                    ...
                    <table class="table table-hover table-striped table-bordered">
                        ...
                        <tbody>
                            <?php 
                                ...
                                $vehicles = $conn->query("SELECT *,Concat(code,' - ',`name`) as vehicle FROM `vehicle_list` where id in (SELECT vehicle_id FROM `schedule_list` where delete_flag = 0 {$swhere})");
                                ...
                                while($row = $qry->fetch_assoc()):
                                    $fc_capacity = isset($vehicle_fcf_arr[$row['vehicle_id']]) ? $vehicle_fcf_arr[$row['vehicle_id']] : 0;
                                    $e_capacity = isset($vehicle_ef_arr[$row['vehicle_id']]) ? $vehicle_ef_arr[$row['vehicle_id']] : 0;
                            ...
                            ?>
                                <tr>
                                    ...
                                    <td class="px-1"><?php echo isset($vehicle_arr[$row['vehicle_id']]) ? $vehicle_arr[$row['vehicle_id']] : "N/A" ?></td>
                                    <td class="px-0">
                                        <div class="px-1 border-bottom"><span class="text-muted">Seater:</span> <span class="text-muted fa fa-user"></span> <b><?= $row['type'] == 1 & 2 ? "<i class='fa fa-question' title='Slot depends to the date you desire.'></i>" : number_format($fc_slot) ?></b> <span class="text-muted ml-2 fa fa-tag"></span> <b><?= rtrim(number_format($row['first_class_fare'],2),'.') ?></b></div>
                                        
                                        <div class="px-1"><span class="text-muted">Shuttle:</span> <span class="text-muted fa fa-user"></span> <b><?= $row['type'] == 1 & 2 ? "<i class='fa fa-question' title='Slot depends to the date you desire.'></i>" : number_format($e_slot) ?></b> <span class="text-muted ml-2 fa fa-tag"></span> <b><?= rtrim(number_format($row['economy_fare'],2),'.') ?></b></div>
                                    </td>
                                    <td class="px-1" align="center">
                                        <a href="./?page=reserve&sid=<?= $row['id'] ?>" class="btn btn-flat btn-primary btn-sm" style="background-color: green;">Book <i class="fa fa-angle-right"></i></a>
                                    </td>
                                </tr>
                            <?php endwhile; ?>
                        </tbody>
