...
<div class="card card-outline card-primary rounded-0 shadow" style="border-color: green;">
	<div class="card-header">
		...
		<div class="card-tools">
			<a href="javascript:void(0)" id="create_new" class="btn btn-flat btn-sm btn-primary" style="background-color: green;"><span class="fas fa-plus"></span>  Add New Schedule</a>
		</div>
	</div>
	<div class="card-body">
        <div class="container-fluid">
			...
			<table class="table table-hover table-striped table-bordered">
				...
				<tbody>
					<?php 
						$i = 1;
						$vehicles = $conn->query("SELECT *,Concat(code,' - ',`name`) as vehicle FROM `vehicle_list` where id in (SELECT vehicle_id FROM `schedule_list` where delete_flag = 0 and `type` = '{$type}')");
					...
					?>
						<tr>
							...
							<td class="px-0">
								<div class="px-1 border-bottom"><span class="text-muted">Seater:</span> <span class="text-muted fa fa-user"></span> <b><?= isset($vehicle_fcf_arr[$row['vehicle_id']]) ? $vehicle_fcf_arr[$row['vehicle_id']] : 0 ?></b> <span class="text-muted ml-2 fa fa-tag"></span> <b><?= rtrim(number_format($row['first_class_fare'],2),'.') ?></b></div>
								<div class="px-1"><span class="text-muted">Shuttle:</span> <span class="text-muted fa fa-user"></span> <b><?= isset($vehicle_ef_arr[$row['vehicle_id']]) ? $vehicle_ef_arr[$row['vehicle_id']] : 0 ?></b> <span class="text-muted ml-2 fa fa-tag"></span> <b><?= rtrim(number_format($row['economy_fare'],2),'.') ?></b></div>
							</td>
