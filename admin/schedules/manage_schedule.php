...
<div class="container-fluid">
    <form action="" id="schedule-form">
        ...
        <div class="form-group">
            <label for="train_id" class="control-label">Vehicle</label>
            <select name="vehicle_id" id="vehicle_id" class="form-control form-control-border select2" >
                <?php 
                $vehicles = $conn->query("SELECT * FROM vehicle_list where delete_flag = 0 ".(isset($vehicle_id) && !empty($vehicle_id) ? " or id = '{$vehicle_id}' " : "")." order by name asc");
                while($row = $vehicles->fetch_assoc()):
                ?>
                <option value="<?= $row['id'] ?>" <?= isset($vehicle_id) && $vehicle_id == $row['id'] ? "selected" : "" ?> <?= $row['delete_flag'] == 1 ? "disabled" : "" ?>><?= ($row['code']. " - ". $row['name']) ?></option>
                <?php endwhile; ?>
            </select>
        </div>
        <fielset>
        ...
        <fielset>
            ...
            <div class="row">
                <div class="form-group col-md-6">
                    <label for="first_class_fare" class="control-label">Seater</label>
                ...
                <div class="form-group col-md-6">
                    <label for="economy_fare" class="control-label">Shuttle</label>
...
<script>
    $(function(){
        ...
        $('#uni_modal').on('shown.bs.modal',function(){
            $('#train_id').select2({
                placeholder:"Please Select vehicle here.",
                width:'100%',
