...
if(isset($_GET['id'])){
    $vehicles = $conn->query("SELECT *,Concat(code,' - ',`name`) as vehicle FROM `vehicle_list` where id in (SELECT vehicle_id FROM `schedule_list` where delete_flag = 0 and id in (SELECT schedule_id FROM `reservation_list` where id in ({$_GET['id']}) ) )");
    ...
    $qry = $conn->query("SELECT r.*,s.code as sched_code,s.vehicle_id from `reservation_list` r inner join `schedule_list` s on r.schedule_id = s.id where r.id in ({$_GET['id']}) ");
    if($qry->num_rows > 0){
