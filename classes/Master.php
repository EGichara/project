...
Class Master extends DBConnection {
	...
	function save_train(){
		...
		$check = $this->conn->query("SELECT * FROM `vehicle_list` where `code` = '{$code}' and delete_flag = 0 ".($id > 0 ? " and id != '{$id}'" : ""));
		if($check->num_rows > 0){
			$resp['status'] = 'failed';
			$resp['msg'] = "vehicle  already exists.";
		}else{
			$save = $this->conn->query($sql);
			if($save){
				$rid = !empty($id) ? $id : $this->conn->insert_id;
				$resp['status'] = 'success';
				if(empty($id))
					$resp['msg'] = "vehicle has been successfully added.";
				else
					$resp['msg'] = "vehicle details has been updated successfully.";
			}else{
				$resp['status'] = 'failed';
				$resp['msg'] = "An error occured.";
				$resp['err'] = $this->conn->error."[{$sql}]";
	...
	function delete_train(){
		...
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success',"Vehicle has been deleted successfully.");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
	...
	function save_reservation(){
		...
		extract($_POST);
		$capacity = $this->conn->query("SELECT `".($seat_type == 1 ? "first_class_capacity" : "economy_capacity")."` FROM vehicle_list where id in (SELECT vehicle_id FROM `schedule_list` where id ='{$schedule_id}') ")->fetch_array()[0];
